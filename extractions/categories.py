...
import requests
from extractions.engines_trims import find_or_create
...
def get_part(link, name, sub_category):
    ...
    if r.status_code < 400:
        ...
        try:
            ...
            created, part = find_or_create(Part, part, session)
            sub_category.parts.append(part)
            session.commit()
...
if __name__ == '__main__':
    ...
    categories_soup = soup.select('#parts-menu > .page-builder-layout-module > div')
    i = 0
    for category in categories_soup:
        ...
        created_cat, new_cat = find_or_create(Category, new_cat, session)
        ...
        for subcategory in subcategories:
            ...
            created_sub, new_sub = find_or_create(Subcategory, newsub, session)
